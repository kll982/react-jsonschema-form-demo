...
function switchTheme(
  ...
  setIPAThemes(newTheme);
}

function ThemeSwitcher(props: { default?: ThemeTypes.ThemeKey }) {
  const currTheme = useRef<ThemeTypes.ThemeKey>(getIPAThemes());
  ...
  useEffect(() => {
    console.log("currTheme.current", currTheme.current);
  }, [currTheme.current]);

  useEffect(() => {
    console.log("getIPAThemes()", getIPAThemes());
  }, [localStorage.getItem("curTheme")]);
